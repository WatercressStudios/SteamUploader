...
init python:
    import os, glob, shutil, zipfile, json, threading
...
"""
    ...
    def SteamUploader_Execute():
        ...
        global steam_username, steam_password, uploading_message
        ...
        for k in project_depot_id_dlc:
            open("{0}/depot_{1}.vdf".format(scripts_dir, project_depot_id_dlc[k]), "w").write(depot_template.format(
                depotid=project_depot_id_dlc[k],
                depotname=project_dlc_files[k-1],
                contentpath="{0}/{1}".format(content_dir, project_dlc_files[k-1]))
            )
                        ...
                        for dlc in project_dlc_files:
                            ...
                            hbox:
                                ...
                                if project_input_editing == "depotid_dlc_"+str(i):
                                    input:
                                        default project_depot_id_dlc.get(i, "")
                                        allow "0123456789"
                                        length 12
                                        copypaste True
                                        changed SteamUploader_ChangeDepotID(i)
                                else:
                                    textbutton project_depot_id_dlc.get(i, ""):
                                        xysize (200, 30)
